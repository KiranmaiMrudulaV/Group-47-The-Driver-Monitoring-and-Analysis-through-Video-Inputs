...
from constants import MAX_SEQUENCE_LENGTH_LIST
...
def train_model(model:Model, dataset_id, dataset_prefix, epochs=50, batch_size=128, val_subset=None,
    ...
    model_checkpoint = ModelCheckpoint("./weights/%s_weights.h5" % dataset_prefix, verbose=1,
                                       monitor= 'auc_roc', save_best_only=True, save_weights_only=True)
    ...
    model.compile(optimizer=optm, loss='categorical_crossentropy', metrics=[auc_roc])
...
def fit_model(model:Model, dataset_id, dataset_prefix, batch_size=128, test_data_subset=None,
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, batch_size=128, test_data_subset=None,
    ...
    optm = Adam(lr=1e-3)
    model.compile(optimizer=optm, loss='categorical_crossentropy', metrics=[auc_roc]) # metrics=[auc_roc]
    ...
    print("\nEvaluating : ")
    loss, auc = model.evaluate(X_test, y_test, batch_size=batch_size)
    print()
    print("Final AUC : ", (1-auc)) #AUC is 1- or just the regular?
    
    return (1-auc) #AUC is 1- or just the regular?
